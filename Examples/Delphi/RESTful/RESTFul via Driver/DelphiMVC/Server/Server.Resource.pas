...
begin
  try
    try
      ...
      Render(TJSONString.Create('Dados exclu�dos do banco com sucesso!!!').ToJSON);
    finally
      if LMaster <> nil then
        LMaster.Free;
    end;
...
begin
  try
    LMaster := TJanusJson.JsonToObject<Tmaster>(AValue);
    try
      if LMaster <> nil then
...
begin
  try
    ...
    /// Retorna o JSON
    /// </summary>
    Render(TJanusJson.ObjectListToJsonString<Tmaster>(LMasterList));
  finally
...
begin
  try
    LMaster := FMaster.Find(AID);
    /// <summary> Retorna o JSON </summary>
    Render(TJanusJson.ObjectToJsonString(LMaster));
  finally
    if LMaster <> nil then
      LMaster.Free;
  end;
...
begin
  try
    LMasterNew := TJanusJson.JsonToObject<Tmaster>(AValue);
    try
...
begin
  try
    ...
    /// <summary> Retorna o JSON </summary>
    Render(TJanusJson.ObjectListToJsonString<Tlookup>(LMasterList));
  finally
    if LMasterList <> nil then
      LMasterList.Free;
  end;
