...
var
  ...
  LAfterInsert: TEvent;
begin
  LBeforeDelete := TJanusMiddlewares.ExecuteEventCallback(AObject, 'BeforeDelete');
  if Assigned(LBeforeDelete) then
    LBeforeDelete(AObject);
  Resolver<T>.Delete(AObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(AObject, 'AfterDelete');
...
begin
  if not FOwnerNestedList then
    Exit;
  SelectNestedListItem<T>;
  LBeforeDelete := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'BeforeDelete');
  if Assigned(LBeforeDelete) then
    LBeforeDelete(FSelectedObject);
  Resolver<T>.Delete(FSelectedObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'AfterDelete');
  if Assigned(LAfterInsert) then
...
begin
  if not FOwnerNestedList then
    Exit;
  SelectNestedListItem<T>;
  LBeforeUpdate := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'BeforeUpdate');
  if Assigned(LBeforeUpdate) then
    LBeforeUpdate(FSelectedObject);
  Resolver<T>.Update(FSelectedObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'AfterUpdate');
  if Assigned(LAfterInsert) then
    LAfterInsert(FSelectedObject);
end;
...
var
  ...
  LAfterInsert: TEvent;
begin
  LBeforeUpdate := TJanusMiddlewares.ExecuteEventCallback(AObject, 'BeforeUpdate');
  if Assigned(LBeforeUpdate) then
    LBeforeUpdate(AObject);
  Resolver<T>.Update(AObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(AObject, 'AfterUpdate');
  if Assigned(LAfterInsert) then
    LAfterInsert(AObject);
end;
...
begin
  ...
  if not FOwnerNestedList then
    Exit;
  LBeforeInsert := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'BeforeInsert');
  if Assigned(LBeforeInsert) then
    LBeforeInsert(FSelectedObject);
  Resolver<T>.Insert(FSelectedObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(FSelectedObject.ClassType, 'AfterInsert');
  if Assigned(LAfterInsert) then
    LAfterInsert(FSelectedObject);
  Result := FCurrentIndex;
end;
...
begin
  Result := FCurrentIndex;
  LBeforeInsert := TJanusMiddlewares.ExecuteEventCallback(AObject, 'BeforeInsert');
  if Assigned(LBeforeInsert) then
    LBeforeInsert(AObject);
  Resolver<T>.Insert(AObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(AObject, 'AfterInsert');
  if Assigned(LAfterInsert) then
    LAfterInsert(AObject);
  if not FOwnerNestedList then
    Exit;
