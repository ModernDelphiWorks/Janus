unit Janus.Container.ObjectSet;
...
begin
  inherited;
  LBeforeDelete := TJanusMiddlewares.ExecuteEventCallback(M, 'BeforeDelete');
  if Assigned(LBeforeDelete) then
    LBeforeDelete(AObject);
  FObjectSetAdapter.Delete(AObject);
  LAfterDelete := TJanusMiddlewares.ExecuteEventCallback(M, 'AfterDelete');
...
begin
  inherited;
  LBeforeInsert := TJanusMiddlewares.ExecuteEventCallback(M, 'BeforeInsert');
  if Assigned(LBeforeInsert) then
    LBeforeInsert(AObject);
  FObjectSetAdapter.Insert(AObject);
  LAfterInsert := TJanusMiddlewares.ExecuteEventCallback(M, 'AfterInsert');
...
begin
  inherited;
  LBeforeUpdate := TJanusMiddlewares.ExecuteEventCallback(M, 'BeforeUpdate');
  if Assigned(LBeforeUpdate) then
    LBeforeUpdate(AObject);
  FObjectSetAdapter.Update(AObject);
  LAfterUpdate := TJanusMiddlewares.ExecuteEventCallback(M, 'AfterUpdate');
