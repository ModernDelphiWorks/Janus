...
type
  IBind = interface
    ...
    procedure SetFieldToField(const AResultSet: IDBDataSet;
      const ADataSet: TDataSet);
    ...
    procedure SetFieldToProperty(const ADataSet: IDBDataSet;
      const AObject: TObject); overload;
...
begin
  ...
  //    LField.Origin := AObject.GetTable.Name + '.' + LFieldName;
    ...
    on E: Exception do
    begin
      raise Exception.Create('Janus.Bind->SetDataDictionary()'
                            + sLineBreak
                            + sLineBreak
                            + 'Column: ' + LFieldName);
    end;
...
procedure TBind.SetFieldToField(const AResultSet: IDBDataSet;
  const ADataSet: TDataSet);
var
  LFor: Integer;
  LFieldValue: Variant;
...
begin
  try
    ...
    begin
      ...
      try
        ...
        begin
          LFieldValue := AResultSet.FieldValues[LField.FieldName];
  ...
  //                    end;
            ...
            LField.Value := AResultSet.FieldValues[LField.FieldName];
          end
        end;
      finally
        LField.ReadOnly := LReadOnly;
...
procedure TBind.SetFieldToProperty(const ADataSet: IDBDataSet;
  const AObject: TObject);
var
  LColumn: TColumnMapping;
  LColumns: TColumnMappingList;
