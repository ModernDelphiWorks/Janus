...
{ @abstract(Janus Framework - the REST ObjectSet family reads the key the server
  ...
  WHAT THIS FIXTURE DOES NOT PIN - FIVE SURVIVING MUTATIONS, LISTED IN FULL
  ...
  errors=0 failures=0 - green, meaning nothing here notices them:
  ...
  ONE ENUMERATION EXPLAINS ALL FIVE. Under Test/Delphi there are 39 entities
  carrying a [PrimaryKey], 40 key properties among them, and:

    - read-only key property   ZERO of 40  -> the IsWritable guard
    - Int64 key                ZERO in the WHOLE repository, Examples/ included
                                           -> both 64-bit mutations
    - non-numeric key          exactly ONE, TStrMaster (String), and it carries
                               no [Sequence], so ExistSequence is False and the
                               block is never entered for it
                                           -> the string branch and its check

  Every one of the 14 entities that DOES carry a [Sequence] has an Integer key.
  So these are not five reasons; they are one fact about the population.

  CLOSING THEM MEANS INVENTING A FIXTURE for a shape the test tree does not
  have - a key that cannot be written, a 64-bit key, a string key generated by a
  sequence - and a fixture that models something impossible teaches the next
  reader something false. Note that the material for the last of them is not
  imaginary: Examples/ carries String, Double and Nullable keys. What it does
  not carry is one of those TOGETHER WITH a [Sequence].
