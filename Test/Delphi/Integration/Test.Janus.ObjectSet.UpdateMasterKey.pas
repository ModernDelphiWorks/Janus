...
{ @abstract(Janus Framework - the MASTER's key reaching a child added during an
  update, ObjectSet side. Issue #242.)

  WHAT IS UNDER TEST

  TObjectSetAdapter<M>.Update. Its sibling TObjectSetAdapter<M>.Insert reads
  the master's primary key mapping and hands every column of it to
  SetAutoIncValueChilds BEFORE running the cascade, which is what puts the
  master's key into the foreign key property of each child about to be
  written. Update runs the cascade and never performs that step, so a child
  that only exists in the edited object - a detail line the stored master did
  not have - is inserted with its foreign key at whatever the caller left in
  it, which for a freshly constructed object is zero. Nothing is raised.

  This is a DIFFERENT site from the one Test.Janus.ObjectSet.CascadeUpdateList
  measures. There the key that fails to travel is the CHILD's own key, on its
  way to the grandchildren, and the site is inside the cascade handler in
  TObjectSetBaseAdapter<M>. Here the key that fails to travel is the MASTER's,
  on its way to its direct children, and the site is the update entry point.
...
unit Test.Janus.ObjectSet.UpdateMasterKey;
...
type
  [TestFixture]
  TTestObjectSetUpdateMasterKey = class
  private
...
const
  cDBFILE     = 'janus_objectset_updatemasterkey.db';
  cROOTTAG    = 'root';
...
{ TTestObjectSetUpdateMasterKey }

procedure TTestObjectSetUpdateMasterKey.Setup;
begin
  FDbFile := cDBFILE;
...
procedure TTestObjectSetUpdateMasterKey.TearDown;
begin
  FNew.Free;
  FNew := nil;
  FOld.Free;
...
function TTestObjectSetUpdateMasterKey._ScalarInt(
...
procedure TTestObjectSetUpdateMasterKey._UpdateTheRootWithABrandNewBranch;
var
  LMid: TAsymTreeMid;
begin
...
procedure TTestObjectSetUpdateMasterKey.Premise_TheBranchIsWrittenByTheInsertLegOfCascadeUpdate;
begin
...
procedure TTestObjectSetUpdateMasterKey.TheMasterKeyMustReachTheBranchAddedOnUpdate;
begin
  _UpdateTheRootWithABrandNewBranch;
...
procedure TTestObjectSetUpdateMasterKey.TheMasterKeyMustReachTheBranchRowAddedOnUpdate;
begin
...
initialization
  TDUnitX.RegisterTestFixture(TTestObjectSetUpdateMasterKey);
